...
3. **Admin Pages**: Dashboard (interactive with RT filter, Recharts visualizations: radar, treemap, stacked bar, gauge, pie, area, composed; popup detail with AI insight; pengangguran tracking; angka capaian; highlight konten; monthly snapshot trends with "Indeks Kemajuan RW" composite score; per-metric trend charts with 10 switchable metrics; auto-updates snapshot on data changes), Kelola KK, Kelola Warga, Kelola Laporan, Kelola Surat, Surat RW (Surat Sakti), Arsip Surat, Edit Profil approval, Bansos Management, Donasi, Keuangan (Kas RW), WA Blast, Kelola Pemilik Kost, Kelola Warga Singgah, Kelola Usaha
