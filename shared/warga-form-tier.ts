...
export function applyDerivedWargaFields<T extends WargaTierFormSlice & { lansia?: boolean; kategoriUmur?: string | null }>(
  form: T,
): T & { lansia: boolean; kategoriUmur: string } {
  const age = getWargaAge(form.tanggalLahir);
  return {
    ...form,
    lansia: age !== null && age >= 60,
    kategoriUmur: resolveKategoriUmur(form.tanggalLahir),
  };
}
