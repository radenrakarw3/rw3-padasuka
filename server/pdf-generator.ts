...
const logoPath = path.join(process.cwd(), "attached_assets", "RW3-Cimahi-Logo-Green@16x_1772999415502.png");
...
export function generateSuratPDFBuffer(options: {
  nomorSurat?: string | null;
  isiSurat: string;
  jenisSurat: string;
}): Buffer {
  const { nomorSurat, isiSurat, jenisSurat } = options;
  ...
  if (logoBase64) {
    try { doc.addImage(logoBase64, "PNG", marginLeft + 2, 12, 18, 18); } catch {}
  }
